...
const PurchaseHistory = () => {
  return (
    <div className="container mx-auto p-6 min-h-screen">
      <h1 className="text-4xl font-bold mb-6">Lịch Sử Mua Hàng FPT Shop</h1>
          ...
          {purchaseHistory.map((item) => (
            <tr key={item.id} className="hover:bg-gray-50">
              ...
              <td className="py-4 px-6 border-b">
                ...
                >
                  {item.status === 'Đã giao' && <CheckCircle className="w-8 h-8" />}
                  {item.status === 'Đang xử lý' && <Clock className="w-8 h-8 text-2xl" />}
                  {item.status === 'Đã hủy' && <XCircle className="w-8 h-8 text-2xl" />}
                  {item.status}
                </span>
              </td>
            </tr>
